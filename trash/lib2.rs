...
pub type Command<T> = Pin<Box<dyn Future<Output = T> + Send + 'static>>;
        ...
        M2: Msg,
    {
            ...
            F: Fn(M) -> M2 + Send + 'static,
        {
            ...
            fn stream(self: Box<Self>, event_stream: EventStream) -> MsgStream<M2> {
                ...
                tokio_stream::StreamExt::map(self.raw.stream(event_stream), mapper).boxed()
            }
        }
        let factories = self
            .0
...
impl<M: Msg> Sub<M> {
    pub fn make<F, S>(stream_fn: F) -> Self
    where
        F: FnOnce() -> S + 'static,
        S: Stream<Item = M> + Send + 'static,
    {
        let sub = SubHelper {
            input: (),
            stream_maker: |(), _| stream_fn(),
        };
        Self(vec![Box::new(sub)])
    }

    /// input will be used as the sub identifier
    pub fn make_with<I, F, S>(input: I, stream_fn: F) -> Self
    where
        I: Hash + 'static,
        ...
        S: Stream<Item = M> + Send + 'static,
    {
        let sub = SubHelper {
            input,
            stream_maker: stream_fn,
        };
        Self(vec![Box::new(sub)])
    }
}
pub struct Every<M, F>
where
    M: Msg,
    F: Fn(Instant) -> M,
{
...
{
    pub fn new(interval: Duration, mapper: F) -> Self {
        Self { mapper, interval }
    }
}

impl<M, F> SubFactory<M> for Every<M, F>
where
    M: Msg,
    F: Fn(Instant) -> M + Send + 'static,
{
    ...
    fn stream(self: Box<Self>, _: EventStream) -> MsgStream<M> {
        let duration = self.interval;
        let builder = self.mapper;
        tokio_stream::wrappers::IntervalStream::new(tokio::time::interval(duration))
            .map(move |i| builder(i.into_std()))
            .boxed()
    }
}

pub struct OnKeyPress<M: Msg, F>
where
    F: Fn(crossterm::event::KeyEvent) -> Option<M> + Send + 'static,
{
    mapper: F,
}

pub struct OnTerminalEvent<M, F>
where
    M: Msg,
    F: Fn(TerminalEvent) -> Option<M> + Send + 'static,
{
    mapper: F,
}

impl<M, F> OnTerminalEvent<M, F>
where
    M: Msg,
    F: Fn(TerminalEvent) -> Option<M> + Send + 'static,
{
...
impl<M, F> SubFactory<M> for OnTerminalEvent<M, F>
where
    M: Msg,
    F: Fn(TerminalEvent) -> Option<M> + Send + 'static,
{
    ...
    fn stream(self: Box<Self>, event_stream: EventStream) -> MsgStream<M> {
        tokio_stream::StreamExt::filter_map(event_stream, move |event| match event {
            Event::Terminal(event) => (self.mapper)(event),
            _ => None,
        })
        .boxed()
    }
...
impl<M, F> SubFactory<M> for OnKeyPress<M, F>
where
    M: Msg,
    F: Fn(crossterm::event::KeyEvent) -> Option<M> + Send + 'static,
{
    ...
    fn stream(self: Box<Self>, event_stream: EventStream) -> MsgStream<M> {
        tokio_stream::StreamExt::filter_map(event_stream, move |event| match event {
            Event::Terminal(TerminalEvent::Key(event)) => (self.mapper)(event),
            _ => None,
        })
        .boxed()
    }
...
impl<I, F, S, M> SubFactory<M> for SubHelper<I, F, S, M>
where
    I: Hash + 'static,
    F: FnOnce(I, EventStream) -> S + 'static,
    S: Stream<Item = M> + Send + 'static,
